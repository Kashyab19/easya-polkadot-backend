...
var mnemonicToMiniSecretAsync = async (mnemonic, password = "") => {
  ...
  const result = await pbkdf2Async(sha5122, entropy, salt, { c: 2048, dkLen: 64 });
  return result.slice(0, 32);
};
var mnemonicToMiniSecret = (mnemonic, password = "") => {
  const { entropy, salt } = mnemonicToEntropyAndSalt(mnemonic, password);
  const result = pbkdf2(sha5122, entropy, salt, { c: 2048, dkLen: 64 });
  return result.slice(0, 32);
};
...
var sshash = (data) => {
  return blake2b2(u8aConcatStrict([SS58_PREFIX, data]), { dkLen: 64 });
};
