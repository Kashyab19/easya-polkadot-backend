...
var keccakF1600 = (state) => {
  (0, import_sha3.keccakP)(new Uint32Array(state.buffer, state.byteOffset, Math.floor(state.byteLength / 4)), 24);
};
...
var SecretKey = class _SecretKey {
  ...
  static FromMiniSecret(miniSecret) {
    ...
    const r = (0, import_sha512.sha512)(miniSecret);
...
var RandomGenerator = class {
  GetRandomArrayU8_32() {
    return (0, import_utils.randomBytes)(32);
  }
  GetHardcoded() {
...
var sha256FirstNBits = (entropy) => (0, import_sha256.sha256)(entropy)[0].toString(2).padStart(8, "0").slice(0, entropy.length / 4);
...
var generateMnemonic = (strengthInBits = 128) => {
  return entropyToMnemonic((0, import_utils2.randomBytes)(strengthInBits / 8)).toString().trim();
};
...
var mnemonicToMiniSecretAsync = async (mnemonic, password = "") => {
  ...
  const result = await (0, import_pbkdf2.pbkdf2Async)(import_sha5122.sha512, entropy, salt, { c: 2048, dkLen: 64 });
  return result.slice(0, 32);
};
var mnemonicToMiniSecret = (mnemonic, password = "") => {
  const { entropy, salt } = mnemonicToEntropyAndSalt(mnemonic, password);
  const result = (0, import_pbkdf2.pbkdf2)(import_sha5122.sha512, entropy, salt, { c: 2048, dkLen: 64 });
  return result.slice(0, 32);
};
...
var keccakF16002 = (state) => {
  (0, import_sha32.keccakP)(new Uint32Array(state.buffer, state.byteOffset, Math.floor(state.byteLength / 4)), 24);
};
var STROBE_R = 166;
var FLAG_I = 1;
var FLAG_A = 1 << 1;
...
var TranscriptRngBuilder2 = class {
  ...
  finalize(generateRandomBytes32) {
    const bytes = generateRandomBytes32 ? generateRandomBytes32() : (0, import_utils3.randomBytes)(32);
...
var getChainCode = (str) => {
  ...
  chainCode.set(u8a.length > 32 ? (0, import_blake2b.blake2b)(u8a, { dkLen: 32 }) : u8a, 0);
  return chainCode;
};
...
var sshash = (data) => {
  return (0, import_blake2b2.blake2b)(u8aConcatStrict([SS58_PREFIX, data]), { dkLen: 64 });
};
