...
module.exports = __toCommonJS(full_exports);
...
var Subscription = class {
  ...
  connect(options) {
    ...
    const socket = (0, import_socket.io)(uri, restOptions);
...
var StateQueries = class extends Section {
  async get(args) {
    const { endpoint, module: module2, method } = args;
    const url = `${endpoint}/${module2}/${method}`;
    const response = await this.client.instance({
      method: Method.GET,
      baseURL: this.baseUrl,
      url
  ...
  async execute(params, body) {
    const { endpoint, module: module2, method } = params;
    const url = `${endpoint}/${module2}/${method}`;
    const response = await this.client.instance({
      method: Method.POST,
      baseURL: this.baseUrl,
