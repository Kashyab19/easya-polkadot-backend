...
var Sr25519 = (() => {
  ...
  __export(sr25519_exports, {
    Account: () => Account,
    Sr25519Account: () => Sr25519Account,
    dangerouslyParseUriAndGetFullKeypair: () => dangerouslyParseUriAndGetFullKeypair,
  ...
  function G1(a, b2, c, d, msg, x) {
    const Xl = msg[x], Xh = msg[x + 1];
    let Al = BUF[2 * a], Ah = BUF[2 * a + 1];
    let Bl = BUF[2 * b2], Bh = BUF[2 * b2 + 1];
    ...
    BUF[2 * b2] = Bl, BUF[2 * b2 + 1] = Bh;
    BUF[2 * c] = Cl, BUF[2 * c + 1] = Ch;
    BUF[2 * d] = Dl, BUF[2 * d + 1] = Dh;
  }
  function G2(a, b2, c, d, msg, x) {
    const Xl = msg[x], Xh = msg[x + 1];
    let Al = BUF[2 * a], Ah = BUF[2 * a + 1];
    let Bl = BUF[2 * b2], Bh = BUF[2 * b2 + 1];
    let Cl = BUF[2 * c], Ch = BUF[2 * c + 1];
  ...
  // node_modules/@unique-nft/sr25519/dist/index.mjs
  ...
  var Strobe = class _Strobe {
    ...
    operate(meta, flags, dataConst, starIndex, count, length, more) {
      ...
      const cAfter = (flags & (4 | 1 | 8)) === (4 | 8);
      const cBefore = (flags & 4) !== 0 && !cAfter;
  ...
  var Transcript = class _Transcript {
    ...
    Init(label) {
      ...
      this.AppendMessage(
        textEncoder2.encode("dom-sep"),
        textEncoder2.encode(label)
      );
    }
    GetStrobe() {
  ...
  function MontgomeryReduce(limbs) {
    ...
    const n4 = _part1(
      n3.i1 + limbs[4] + _m(n0.i0, BigInt(l[4])) + _m(n2.i0, BigInt(l[2])) + _m(n3.i0, BigInt(l[1]))
    );
    const r0 = _part2(
      n4.i1 + limbs[5] + _m(n1.i0, BigInt(l[4])) + _m(n3.i0, BigInt(l[2])) + _m(n4.i0, BigInt(l[1]))
    );
    const r1 = _part2(
  ...
  function readUint8ArrayIntoBigIntArray(bytes2) {
    const bigInts = [];
    for (let i = 0; i < bytes2.length; i += 8) {
      let num = 0n;
      for (let j = i + 7; j >= i; j--) {
        num = num << 8n | BigInt(bytes2[j]);
      }
      bigInts.push(num);
    }
    return bigInts;
  ...
  var Scalar = class _Scalar {
    ...
    static FromBits(bytes2) {
      if (bytes2.length !== 32) {
        throw new Error(`Invalid size of data: should be 32, got ${bytes2.length}`);
      }
      const s = new _Scalar();
      s.bytes = bytes2;
      s.bytes[31] &= 127;
    ...
    static ToRadix16(bytes2) {
      const output2 = [];
      for (let i = 0; i < 32; i++) {
        output2[2 * i] = bytes2[i] & 15;
        output2[2 * i + 1] = bytes2[i] >> 4 & 15;
      }
      for (let i = 0; i < 63; i++) {
        const carry = output2[i] + 8 >> 4;
        output2[i] -= carry << 4;
        output2[i + 1] += carry;
      }
      return output2;
    }
    static DivideScalarBytesByCofactor(bytes2) {
      const res = new Uint8Array(bytes2.length);
      let low = 0;
    ...
    static MultiplyScalarBytesByCofactor(bytes2) {
      const res = new Uint8Array(bytes2.length);
      let high = 0;
      for (let i = 0; i < bytes2.length; i++) {
        const r = bytes2[i] & 224;
        bytes2[i] = bytes2[i] << 3;
        bytes2[i] += high;
        res[i] = bytes2[i];
        high = r >> 5;
  ...
  var FieldElement = class _FieldElement {
    ...
    static FromBytes(bytes2) {
      const res = this.Zero();
      res.data[0] = uint8ArrayToBigIntLE(bytes2.slice(0, 8)) & LOW_51_BIT_MASK;
      res.data[1] = uint8ArrayToBigIntLE(bytes2.slice(6, 6 + 8)) >> 3n & LOW_51_BIT_MASK;
      res.data[2] = uint8ArrayToBigIntLE(bytes2.slice(12, 12 + 8)) >> 6n & LOW_51_BIT_MASK;
      res.data[3] = uint8ArrayToBigIntLE(bytes2.slice(19, 19 + 8)) >> 1n & LOW_51_BIT_MASK;
      res.data[4] = uint8ArrayToBigIntLE(bytes2.slice(24, 24 + 8)) >> 12n & LOW_51_BIT_MASK;
      return res;
    }
    Clone() {
    ...
    Mul(second) {
      ...
      const output2 = Array(5);
      c1 += c0 >> 51n;
      output2[0] = (c0 & LOW_51_BIT_MASK) % U64size;
      c2 += c1 >> 51n;
      output2[1] = (c1 & LOW_51_BIT_MASK) % U64size;
      c3 += c2 >> 51n;
      output2[2] = (c2 & LOW_51_BIT_MASK) % U64size;
      c4 += c3 >> 51n;
      output2[3] = (c3 & LOW_51_BIT_MASK) % U64size;
      const carry = (c4 >> 51n) % U64size;
      output2[4] = (c4 & LOW_51_BIT_MASK) % U64size;
      output2[0] = output2[0] + carry * 19n;
      output2[1] += output2[0] >> 51n;
      output2[0] &= LOW_51_BIT_MASK;
      return new _FieldElement(output2);
    }
    Reduce(limbs) {
  ...
  var AffineNielsPoint = class _AffineNielsPoint {
    ...
    static FromArray(array) {
      ...
      return _AffineNielsPoint.FromElements(
  ...
  var EDWARDS_D2 = new FieldElement([
    1859910466990425n,
    932731440258426n,
    1072319116312658n,
    1815898335770999n,
  ...
  var EDWARDS_D = new FieldElement([
    929955233495203n,
    466365720129213n,
    1662059464998953n,
    2033849074728123n,
  ...
  var EdwardsPoint = class _EdwardsPoint {
    ...
    static FromCompressedPoint(bytes2) {
      const s = FieldElement.FromBytes(bytes2);
      if (s.IsNegative()) {
    ...
    static Identity() {
      return _EdwardsPoint.EdwardsPointFromElems(
        FieldElement.Zero(),
        FieldElement.One(),
        FieldElement.One(),
        FieldElement.Zero()
  ...
  var _LookupTable = class _LookupTable2 {
    ...
    static FromArrayOfStringsInRadix36Form(table) {
      return _LookupTable2.FromANPArray(table.map(AffineNielsPoint.FromStringInRadix36Form));
    }
    // x - sbyte
    Select(x) {
      const xmask = x >> 7;
    ...
    static From(ep) {
      return _LookupTable2.FromEdward(ep);
    }
  };
  ...
  var ED25519_BASEPOINT_TABLE_INNER = new EdwardsBasepointTable(
    ed25519BasepointLookupTablesInStringRadix36Form.map(LookupTable.FromArrayOfStringsInRadix36Form)
  );
  var INVSQRT_A_MINUS_D = new FieldElement([
    278908739862762n,
    821645201101625n,
    8113234426968n,
    1777959178193151n,
  ...
  var RistrettoPoint = class _RistrettoPoint {
    ...
    static FromCompressedPoint(compressed) {
      const bytes2 = compressed.ToBytes();
      const ep = EdwardsPoint.FromCompressedPoint(bytes2.slice());
      return new _RistrettoPoint(ep);
    }
    static FromCompressedPointBytes(bytes2) {
      const ep = EdwardsPoint.FromCompressedPoint(bytes2.slice());
      return new _RistrettoPoint(ep);
    }
    Negate() {
  ...
  var SecretKey = class _SecretKey {
    ...
    static FromBytes(bytes2) {
      if (bytes2.length !== 64) {
        throw new Error(`Invalid secret key length: ${bytes2.length} (expected 64)`);
      }
      const secretKey = new _SecretKey();
      secretKey.key = Scalar.FromBytes(
        Scalar.DivideScalarBytesByCofactor(bytes2.slice(0, 32))
      );
      secretKey.nonce = bytes2.slice(32, 64);
      return secretKey;
    }
    ...
    ToBytes() {
      ...
      bytes2.set(key, 0);
      bytes2.set(this.nonce.slice(), 32);
      return bytes2;
    }
    ToPublicKey() {
  ...
  var PublicKey = class _PublicKey {
    ...
    verify(message, signatureBytes) {
      ...
      const k = Scalar.FromBytes(Scalar.FromBytesModOrderWide(
        signingTranscript.ChallengeBytes(textEncoder3.encode("sign:c"), 64)
      ));
      const A = this.ToRistrettoPoint();
      const negA = A.Negate();
      const R2 = RistrettoPoint.vartimeDoubleScalarMulBasepoint(
        k,
        negA.Ep,
        signature.S
      );
  ...
  var Signature = class _Signature {
    ...
    static FromBytes(bytes2) {
      ...
      const lower = bytes2.slice(0, 32);
      const upper = bytes2.slice(32, 64);
      if ((upper[31] & 128) === 0) {
  ...
  var SigningTranscript = class {
    ...
    SetProtocolName(label) {
      this.CommitBytesB(
        ...
        textEncoder3.encode("proto-name"),
        label
      );
    }
    ...
    CommitPointBytes(label, bytes2) {
      this.CommitBytesB(this.context.GetTranscript(), label, bytes2);
    }
    WitnessScalarLabel(label, bytes2, rng) {
      return this.WitnessScalarFR(this.context.GetTranscript(), label, bytes2, rng);
    }
    WitnessScalar(bytes2, rng) {
      return this.WitnessScalarSR(this.context.GetTranscript(), bytes2, rng);
    }
    ...
    CommitBytesB(ts, label, bytes2) {
      ts.AppendMessage(label, bytes2);
    }
    CommitBytesS(ts, label, bytes2) {
      ts.AppendMessage(textEncoder3.encode(label), bytes2);
    }
  ...
  var bigIntToUint8Array = (value, options = DEFAULT_OPTS) => {
    ...
    const byteLength = Math.ceil(
      bitLength === -1 ? (value || 0n).toString(2).length / 8 : (bitLength || 0) / 8
  ...
  var uInt8ArrayToHex = (bytes2) => {
    return !bytes2 ? "0x" : bytes2.reduce((str, byte) => str + byte.toString(16).padStart(2, "0"), "0x");
  };
  var hexStringToUint8Array = (str, bitLength = -1) => {
    ...
    const bytes2 = pairs.map((b2) => parseInt(b2, 16));
  ...
  var maxU32 = 1023;
  var bl16 = { bitLength: 16 };
  var bl32 = { bitLength: 32 };
  ...
  var TranscriptRngBuilder2 = class {
    ...
    finalize(generateRandomBytes32) {
      const bytes2 = generateRandomBytes32 ? generateRandomBytes32() : randomBytes(32);
      this.strobe.meta_ad(b`rng`, false);
      this.strobe.key(bytes2, false);
  ...
  var parseUri = (uri) => {
    ...
    for (const part of hardSeparatedParts) {
      ...
      if (!hard)
        throw new Error("Invalid hard derivation");
  ...
  var Keypair = class _Keypair {
    ...
    static FromBytes(bytes2) {
      const secretKey = SecretKey.FromBytes(bytes2.slice(0, 64));
      const publicKey = PublicKey.FromBytes(bytes2.slice(64));
    ...
    ToBytes() {
      const bytes2 = new Uint8Array(96);
      bytes2.set(this.secretKey.ToBytes());
      bytes2.set(this.publicKey.key, 64);
      return bytes2;
    }
  };
  ...
  var checkAddressChecksum = (decoded, ignoreChecksum = false) => {
    ...
    if (!ignoreChecksum) {
      ...
      isValid = (decoded[0] & 128) === 0 && ![46, 47].includes(decoded[0]) && (isPublicKey ? decoded[decoded.length - 2] === hash2[0] && decoded[decoded.length - 1] === hash2[1] : decoded[decoded.length - 1] === hash2[0]);
    }
  ...
  return __toCommonJS(sr25519_exports);
})();
